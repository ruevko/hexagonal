...
EC_COVID19 = data.frame(
   fuente = c(rep("Salud_Ec", 9), rep("Riesgos_Ec", 18)),
   status = c(
      "1233774191326957575",
      "1234331734889697281",
      "1234670644878462980",
   ...
   corte = as.POSIXct(c(
      ...
      "2020-03-23 10:00"), #19
      "America/Guayaquil"),
   cerco = c(rep(NA, 9),279,279,317,273,273,273,315,451,NA,575,575,746,746,1213,1213,1213,1347,1347),
   negativo = c(rep(NA, 12), 51, 51, 51, 51, 51, 278, 312, 336, 412, 483, 533, 601, 649, 872, 1091),
   positivo=c(1,6,7,10,13:15,17,19,23,23,28,37,37,58,58,111,155,168,199,260,367,426,506,532,789,981),
   fallecido = c(rep(0, 9), 1, 1, rep(2, 7), 3, 3, 4, 5, 7, 7, 7, 14, 18))
...
```{r table}
EC_COVID19 = EC_COVID19[-c(11,14,16,19,21,23,25), c(1,4,7,8)]
...
ggsave("ec_covid19.jpg", dpi = "screen", width = 16, height = 7.925, plot = {
   plot(EC_incid) + theme_minimal() + scale_y_sqrt() +
...
La primera alternativa arroja `r round(R_viruses$R_ml, 2)` como el valor más probable; es posible considerar este valor una sobrestimación de R~0~ según lo observado en estudios recientes. Mientras tanto, la segunda alternativa estima un R~0~ = `r round(R_chains$R_ml, 2)` (95 % CI: `r R_sample = sample_R(R_chains, 1e4); paste(round(quantile(R_sample, c(.025, .975)), 2), collapse = " - ")`); un valor congruente con los estudios, donde se ha observado que el SARS-CoV-2 posee un SI más corto que otros coronavirus. Por ende, aun con un ritmo reproductivo menor, puede generar más casos.
...
add_projections(plot(EC_incid), EC_proj + max(EC_incid$counts), c(.025, .5)) +
   ...
   scale_y_continuous("Incidencia diaria acumulada", 1e3 * 0:13) +
   scale_fill_manual(values = "firebrick")
```
...
EC_proj = project(EC_incid, R_chains$R_ml/2, R_chains$si, 1e4, 14) %>% cumulate()

plot(EC_incid) %>% add_projections(EC_proj + max(EC_incid$counts), c(.025, .5)) +
   geom_vline(xintercept = as.Date("2020-03-17"), col = "gold", alpha = .6, size = 2) +
   scale_x_date(date_breaks = "4 days", date_labels = "%b %e") +
...
```{r project-best, fig.asp=.5, fig.cap=paste("Proyección de incidencia diaria acumulada de COVID-19 en Ecuador si R0 =", round(R_chains$R_ml/5, 4))}
EC_proj = project(EC_incid, R_chains$R_ml/5, R_chains$si, 1e4, 14) %>% cumulate()
