...
EC_COVID19 = data.frame(
   fuente = c(rep("Salud_Ec", 9), rep("Riesgos_Ec", 25)),
   status = {c(
      "1233774191326957575",
      "1234331734889697281",
      "1234670644878462980",
      ...
      "1242935910028771329",  #23
      "1243194238269034497",  #24
      "1243297352435273730",  #25
      "1243571908878180354")},#26
   sitrep = c(rep(NA, 9), 1:8, 10:26),
   corte = as.POSIXct({c(
      "2020-02-29 10:02",
      "2020-03-01 23:02",
      "2020-03-02 22:01",
      ...
      "2020-03-27 10:00")},#26
      "America/Guayaquil"),
   cerco = c(rep(NA, 9), 279, 279, 317, 273, 273, 273, 315, 451, NA, 575, 575,
             746, 746, 1213, 1213, 1213, 1347, 1347, 1347, NA, NA, NA, NA, NA, NA),
   negativo = c(rep(NA, 12), 51, 51, 51, 51, 51, 278, 312, 336, 412, 483, 533,
                601, 649, 872, 1091, 1211, 1225, 1346, 1387, 1676, 1722, 1889),
   positivo = c(1, 6, 7, 10, 13, 14, 15, 17, 19, 23, 23, 28, 37, 37, 58, 58, 111, 155, 168, 199,
                260, 367, 426, 506, 532, 789, 981, 1049, 1082, 1173, 1211, 1382, 1403, 1595),
   fallecido =c(rep(0, 9), 1, 1, rep(2, 7), 3, 3, 4, 5, 7, 7, 7, 14, 18, 27, 27, 28, 29, 34, 34, 36))
...
```{r table}
EC_COVID19 = EC_COVID19[-c(11, 14, 16, 19, 21, 23, 25, 29, 31, 33), c(1, 4, 7, 8)]
...
Estos datos representan la incidencia promedio cada día, en un primer escenario donde R~0~ no cambia en absoluto. Los casos actuales, exceptuando la primera semana, y la proyección futura -la región sombreada delimita el 95 % CI- se presentan en los siguientes gráficos de manera acumulativa:
...
EC_cum = cumulate(EC_incid)[-c(1:7)]

EC_proj = cumulate(EC_proj) + EC_cum$counts[16]

add_projections(plot(EC_cum), EC_proj, quantiles = c(.025, .5)) +
   geom_vline(xintercept = as.Date("2020-03-17"), col = "gold", alpha = .6, size = 2) +
...
EC_proj = project(EC_incid[1:23], R_sample/2, R_chains$si, 1e4, 14) %>% cumulate()

plot(EC_cum) %>% add_projections(EC_proj + EC_cum$counts[16], quantiles = c(.025, .5)) +
   geom_vline(xintercept = as.Date("2020-03-17"), col = "gold", alpha = .6, size = 2) +
   scale_y_continuous("Incidencia diaria acumulada", 400 * 0:9) +
   scale_x_date(date_breaks = "4 days", date_labels = "%b %e") +
   scale_fill_manual(values = "firebrick")
...
```{r project-best, fig.asp=.5, fig.cap=paste("Proyección de incidencia diaria acumulada de COVID-19 en Ecuador si R0 =", round(R_chains$R_ml/4, 2))}
EC_proj = project(EC_incid[1:23], R_sample/4, R_chains$si, 1e4, 14) %>% cumulate()

plot(EC_cum) %>% add_projections(EC_proj + EC_cum$counts[16], quantiles = c(.025, .5)) +
   geom_vline(xintercept = as.Date("2020-03-17"), col = "gold", alpha = .6, size = 2) +
   scale_y_continuous("Incidencia diaria acumulada", 200 * 0:9) +
   scale_x_date(date_breaks = "4 days", date_labels = "%b %e") +
   scale_fill_manual(values = "firebrick")
