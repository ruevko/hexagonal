...
```{r model2, fig.asp=1.2, fig.cap="Regresiones lineales :chart_with_upwards_trend: ajustadas en países seleccionados"}
...
select(countries_model, continent:data) %>%
   ...
   geom_point(aes(y = CumNodes), size = .6, show.legend = FALSE) +
...
select(my_model, continent:data) %>%
   ...
   facet_wrap(~country, ncol = 3, scales = "free_y") +
   geom_point(aes(y = CumNodes), size = 1, show.legend = FALSE) +
...
World = select(countries_model, country, m, r) %>%
   mutate_at("m", cut, c(0, 1, 2, 5, 10, 20, 50, 200)*1e3, labels =
                c("(50, 1e3]", "(1e3, 2e3]", "(2e3, 5e5]", "(5e3, 10e3]",
...
my_plot(World) +
   ...
   theme(legend.key.width = unit(.03, "npc"), legend.key.height = unit(.045, "npc"))
```
